...
test("only authenticated webhook and browser-auth entry routes bypass authentication", () => {
  ...
  assert.equal(
    publicAuthPath("/api/v1/webhook-sources/github/events"),
    true,
  );
  assert.equal(
    publicAuthPath(
      "/api/v1/webhook-sources/github/callback?code=example&state=example",
    ),
    true,
  );
  assert.equal(
    publicAuthPath("/api/v1/webhook-sources/github/connections"),
    false,
  );
